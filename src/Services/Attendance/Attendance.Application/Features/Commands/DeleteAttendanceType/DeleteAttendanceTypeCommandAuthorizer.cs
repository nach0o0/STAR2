...
{
    public class DeleteAttendanceTypeCommandAuthorizer : ICommandAuthorizer<DeleteAttendanceTypeCommand>
    {
        private readonly IAttendanceTypeRepository _attendanceTypeRepository;

        public DeleteAttendanceTypeCommandAuthorizer(IAttendanceTypeRepository attendanceTypeRepository)
        {
            _attendanceTypeRepository = attendanceTypeRepository;
        }

        public async Task AuthorizeAsync(DeleteAttendanceTypeCommand command, CurrentUser currentUser, CancellationToken cancellationToken)
        {
            ...
            var requiredPermission = AttendanceTypePermissions.Delete;
            var requiredScope = $"{PermittedScopeTypes.EmployeeGroup}:{attendanceType.EmployeeGroupId}";

            if (!currentUser.PermissionsByScope.TryGetValue(requiredScope, out var permissions) ||
                !permissions.Contains(requiredPermission))
            {
                throw new ForbiddenAccessException("Permission to delete attendance types for this employee group is required.");
            }
        }
    }
}
